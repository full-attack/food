...
class _HomeState extends State<HomeScreen> {
  ...
  Future Upload(File imageFile) async {
    ...
    if (response.statusCode == 200) {
      ...
      print(valuesApi);
      void valuesApiPrinter() {
        for (var listLogMeal in valuesApi) {
          print(listLogMeal);
        }
      }
    } else {
  ...
  Widget Futur(BuildContext context) {
          ...
          return Scaffold(
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(
                children: [
                  SizedBox(width: 20),
                  if (snapshot.hasData) Text('${snapshot.data!} ХУЛИ НЕ РАБОТАЕМ АААААААААААА')
                ],
              ),
            ]),
          );
